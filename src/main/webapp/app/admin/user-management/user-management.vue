<template>
  <div>
      ...
      <div class="d-flex justify-content-end">
        ...
        <router-link custom v-slot="{ navigate }" :to="{ name: 'JhiUserCreate' }">
          <button @click="navigate" class="btn btn-primary jh-create-entity">
            <font-awesome-icon icon="plus"></font-awesome-icon> <span>Adicionar</span>
          </button>
        </router-link>
      </div>
    </h2>
    <div class="table-responsive" v-if="users">
      <table class="table table-striped" aria-describedby="Users">
        <thead>
          <tr>
            <th scope="col" v-on:click="changeOrder('id')">
              <span>Código</span> <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'id'"></jhi-sort-indicator>
            </th>
        ...
        <tbody v-if="users">
          <tr v-for="user in users" :key="user.id" :id="user.login">
            ...
            <td>
              <button class="btn btn-danger btn-sm deactivated" v-on:click="setActive(user, true)" v-if="!user.activated">Inativo</button>
              <button
      ...
      <b-modal ref="removeUser" id="removeUser" title="Confirme a exclusão" @ok="deleteUser()">
        <div class="modal-body">
          <p id="jhi-delete-user-heading">Você tem certeza que deseja excluir o usuário {{ removeId }}?</p>
        </div>
        <template #modal-footer>
          <div>
            <button type="button" class="btn btn-secondary" v-on:click="closeDialog()">Cancelar</button>
            <button type="button" class="btn btn-primary" id="confirm-delete-user" v-on:click="deleteUser()">Excluir</button>
          </div>
        </template>
      </b-modal>
    </div>
